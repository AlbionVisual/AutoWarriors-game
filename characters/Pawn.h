 ...
 * @brief Тут описывается абстрактный класс любого персонажа в игре
 * @date 2025-10-02
 *
 */
...
#include <functional>
  ...
  *        последующего использования, как это подробно описывается в:
  *        @see GameDrawer::add_message(string, int, ConsoleColor)
  *
  */
...
struct pawn_specifications {
    ...
    /// @brief Сила; прибавляется к урона
    int strength;
...
public:
     ...
     *
     * Также переписывается в @see Player::get_extra_specifications(const Pawn&, int)
     * и @see Enemy::get_extra_specifications(const Pawn&, int)
     */
