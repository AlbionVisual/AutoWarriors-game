 ...
 *
 * Базовая функция в Pawn::get_extra_specifications(const Pawn&, int), а также есть упрощённый
...
pawn_specifications Player::get_extra_specifications(const Pawn& target, int move_number, bool is_attacking) {
    ...
    for (auto& pl_class : TYPES_VALUES) {
        ...
        const vector<Effects>& all_effects = EFFECT_RELATION.at(pl_class);
        for (int i = 0; i < lvl; i++) {
...
void Player::apply_damage(const Pawn& damager, int extra_damage, int move_number) {
    int health_change = 0;
    // Нужно обновлять здоровье в любом случае, поэтому если ничего не применилось вызывается no_posteffect(const Pawn&, const Pawn&, int, int, printer_type) в конце
    bool post_effect_applied = false;

    for (auto& pl_class : TYPES_VALUES) {
        int lvl = player_classes.at(pl_class);
        const vector<PostEffects>& all_effects = POSTEFFECT_RELATION.at(pl_class);
        for (int i = 0; i < lvl; i++) {
...
/**
 * @brief Получение видимого имени при помощи Players::VISIBLE_NAMES
 *
 ...
 * #### Example:
 *
 * Единственный раз создаётся в main.cpp
 *
 * ```
 ...
 *        при определении константы созданные в PlayerData.h
 *
 * #### Example:
 *
 * Используется только в конструкторах Player::Player() и Player::Player(Players::PlayerTypes)
 *
 */
...
/**
 * @brief Устанавливает здоровье на значение до боя
 *
 * #### Example:
 *
 * Вызывается каждый раз после победы игрока Game::play_round(bool)
 *
 */
 ...
 * #### Example:
 *
 * Используется после победного боя, когда нужно поднять уровень игроку Game::play_round(bool)
 *
 */
 ...
 * #### Example:
 *
 * Используется, когда нужно определить можно ли повышать ещё уровень Game::play_round(bool)
 */
