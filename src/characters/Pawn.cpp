 ...
 * #### Example:
 *
 * Используется при создании врагов Enemy::set_defaults(), когда создаются константы
 * (возможно заметить это будет затруднительно, но в других местах нет примера)
 * Enemies::SPECIFICATIONS. Поле характеристики при помощи оператора = копируется по значениям
 * в поле только что созданного объекта класса Enemy.
 *
 */
 ...
 * #### Example:
 *
 * Используется неявно при создании персонажей Player::Player(). Поле характеристики просто не
 * заполняется, поэтому вызывается этот конструктор по умолчанию
 *
 */
 ...
 * #### Example:
 *
 * Используется при создании врагов Enemy::set_defaults(). Поле характеристики при помощи
 * оператора "=" копируется по значениям в поле только что созданного объекта класса Enemy.
 *
 * ```
 ...
 *        (при помощи полей specifications.agility у первого и второго персонажей), затем
 *        применяются эффекты по-очереди при помощи переписываемых методов
 *        Pawn::apply_damage(const Pawn&, int, int), Pawn::get_extra_specifications(Pawn&, int)
 *        и аналогичных у Player
 *
 ...
 *
 * Расширяется в классах: Player и Enemy. Сами методы:
 * Player::apply_damage(const Pawn&, int, int), Enemy::apply_damage(const Pawn&, int, int)
 *
 */
