 ...
 *
 * @param * Описаны в posteffect_func_type как общие для группы функций
 *
...
int double_damage_on_crushing(const Pawn& self, const Pawn& damager, int extra_damage, int move_number, printer_type external_printer) {
    ...
    if (damager.active_weapon.damage_type == CRUSHING) {
        if (external_printer) {
            ...
            external_printer("+" + to_string(actual_damage), 2, BLACK_GREEN);
            external_printer(" to damage)", 2, BLACK_WHITE);
        }
        return 2 * actual_damage;
    }
    return actual_damage;
}
 ...
 * и Enemy.cpp в функциях Pawn::get_extra_specifications(const Pawn&, int), где ищется
 * соответствие в константном словаре и при помощи этой функции получается функция-постэффект
 *
